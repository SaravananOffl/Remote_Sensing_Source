...
def main():
    ...
    for file in findFiles(path):
        ...
        soyabean_pixels = np.count_nonzero(band1)
        print(soyabean_pixels,'\n')
        ...
        sheet.write(row_no+1, 9, soyabean_pixels)
        row_no = row_no  +1

        book.save('1June.xls') # <- Output Excel File Name
        print(file)
        # visualise(band4) 
